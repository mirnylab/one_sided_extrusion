# THIS CODE is used for the mixing model of 1-sided and 2-sided extruders
# Unlike entropic_smcTranslocator_inc.pyx, it randomly assigns 1- or 2- sided at each LEF birth event
...
cdef class smcTranslocatorDirectional(object):
    ...
    cdef cython.double prob_onesided
    ...
    def __init__(self, emissionProb, deathProb, stallProbLeft, stallProbRight, pauseProb, stallFalloffProb,  
                 numSmc, onesided,prob_onesided=1, paired=0, switch=0, 
        ...
        self.prob_onesided = prob_onesided # probability of one-sided for all subsequent "births"
        self.paired=paired
        self.slide=slide
    ...
    cdef init_birth(self, cython.int ind):
        cdef int pos,i 
  
        while True:
            pos = np.random.randint(0,self.N//2 - 3) # generate "left SMC" position
            if pos >= self.N - 1:
            ...
            if self.occupied[pos] == 1:
                continue
            if self.occupied[self.N-pos-1] == 1: 
                continue
            ...
            self.SMCs1[ind] = pos
            self.SMCs2[ind] = self.N-pos-1 #don't let smcs start on single binding site
            if self.paired:
                self.SMCs3[ind]= pos + np.random.randint(0,2) #just put the center site on one of those two, don't allow shrinking 
            self.occupied[pos] = 1
            self.occupied[self.N-pos-1] = 1
            
            # randomly select if LEF is one-sided or two-sided
            if randnum() <= self.prob_onesided:
                self.onesided[ind] = 1
            else:
                self.onesided[ind] = 0
                
            # if one-sided, select "active" subunit direction
    ...
    def setInitialPositions(self, smcs1,smcs2):
        self.SMCs1 = smcs1
        self.SMCs2 = smcs2
        self.occupied = np.zeros(self.M, int)
        self.stalled1 = np.zeros(self.M, int)
        self.stalled2 = np.zeros(self.M, int)
        for x in self.SMCs2:
            self.occupied[x] = 1
