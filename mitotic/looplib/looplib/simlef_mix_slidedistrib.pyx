#Modified from simlef_mix to allow the two sides of each LEF to translocate with different velocities 
...
cdef class System:
    ...
    cdef np.int64_t get_leading_leg(System self, np.int64_t leg_idx, np.int64_t direction):
        ...
        if self.vels[leg_idx]==self.vels[leg_idx+2*self.N]:#diffusion
            return 0
        elif ((leg_idx<self.N) and (direction==-1)) or ((leg_idx>=self.N) and (direction==1)): #all non-diffusion of left moving left or right moving right
            return 1
        ...
        if (prev_pos >= 0):
            ...
            if (system.perms[prev_pos + (direction + 1) // 2] > 0): #perms should be 0 at boundaries
                if system.lattice[prev_pos+direction] < 0:
                    status *= system.make_step(leg_idx, direction)  #THIS DETERMINES WHETHER STEP IS MADE! need to grab its info...
...
cpdef simulate(p, verbose=True):
    ...
    #separate sliding arrays if I have a distribution of sliding velocities
    cdef np.float64_t [:] SLIDE_V1 = np.zeros(N, dtype=np.float64)
    cdef np.float64_t [:] SLIDE_V2 = np.zeros(N, dtype=np.float64)

    if (not (type(p['R_SLIDE']) in (list, np.ndarray))) or (not (type(p['R_SLIDE'][0]) in (list, np.ndarray))):
        for i in range(2):
            SLIDING_VEL[i] = p['R_SLIDE'][i] if type(p['R_SLIDE']) in (list, np.ndarray) else p['R_SLIDE']
    else:
        for i in range(N):
            SLIDE_V1[i] = p['R_SLIDE'][0][i]
            SLIDE_V2[i] = p['R_SLIDE'][1][i]
    ...
    for i in range(N):
        ...
        if i < NUM_ONE_SIDED: #sets velocities to zero as needed.
            if (not (type(p['R_SLIDE']) in (list, np.ndarray))) or (not (type(p['R_SLIDE'][0]) in (list, np.ndarray))):
                VELS[i+lags[i]*N] = SLIDING_VEL[lags[i]] # left legs -- so ordering of vels does not correspond to ordering of events
                #looks ok. leg idx is converted from event idx in do_event
                VELS[i+lags[i]*N+2*N] = SLIDING_VEL[(1+lags[i])%2] # right legs
            else:
                if not lags[i]:
                    VELS[i+lags[i]*N] = SLIDE_V1[i]
                    VELS[i+lags[i]*N+2*N] = SLIDE_V2[i]
                else:
                    VELS[i+lags[i]*N] = SLIDE_V2[i]
                    VELS[i+lags[i]*N+2*N] = SLIDE_V1[i]
